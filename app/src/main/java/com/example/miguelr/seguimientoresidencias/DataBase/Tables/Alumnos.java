...
public class Alumnos {
    ...
    public int inicioSession(String usuario,String contrasenia){
        ...
        try{
            String sql = "SELECT iIdAlumno FROM "+getNameTable()+" WHERE "+getvCorreo()+" = '"+usuario+"' OR "+getvMatricula()+" = '"+usuario+"' AND "+getvContrasenia()+" = '"+contrasenia+"'";
    ...
    public Alumnos buscarAlumnoPorId(int idAlumno){
        ...
        try{
            ...
            sql = "SELECT " +
                  ...
                  "INNER JOIN "+carreras.getTableName()+" AS c ON (a."+this.getiIdCarrerafk()+" = "+carreras.getiIdCarrera()+")" +
                  "WHERE a."+this.getiIdAlumno()+" = "+idAlumno;
            Log.d("sql",sql);
            Cursor c = db.rawQuery(sql,null);
